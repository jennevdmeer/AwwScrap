...
namespace AwwScrap.Networking
{
	public static class Messaging
	{
	    ...
	    private static readonly ushort InternalNetworkId = Constants.NetworkId;

		public static void Register()
		{
			MyAPIGateway.Multiplayer.RegisterMessageHandler(InternalNetworkId, MessageHandler);
			MyAPIGateway.Utilities.MessageEntered += ChatMessageHandler;
		}

		public static void Unregister()
		{
			MyAPIGateway.Multiplayer.UnregisterMessageHandler(InternalNetworkId, MessageHandler);
			lock (_playerCache)
			{
				_playerCache = null;
			}
		...
		{
			if (!message.StartsWith(ChatMessages.ChatCommandPrefix))
			{
				sendToOthers = true;
				return;
			}
		...
		public static void SendMessageTo(ulong steamId, MessageBase message, bool reliable = true)
		{
			...
			MyAPIGateway.Multiplayer.SendMessageTo(InternalNetworkId, d, steamId, reliable);
		}
		...
		public static void SendMessageToServer(MessageBase message, bool reliable = true)
		{
			...
			if (!reliable && d.Length >= 1000)
				throw new Exception($"Attempting to send unreliable message beyond message size limits! Message type: {message.GetType()} Content: {string.Join(" ", d)}");
			MyAPIGateway.Multiplayer.SendMessageToServer(InternalNetworkId, d, reliable);
		}
